...
if ! lsof -i :8000; then
  echo "Train service failed to start on port 8000"
  exit 1
fi

if ! lsof -i :3002; then
  echo "Admin service failed to start on port 3002"
  exit 1
fi
...
echo "Train service started successfully on port 8000"
echo "Admin service started successfully on port 3002"
